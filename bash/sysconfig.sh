...
function error-message {
  echo " Error occures and provided to stderr" >&2
}

# This function will send a message to stderr and exit with a failure status
function error-exit {
  echo "Error occured from the script and the failure code is $?"
  exit
}
# Usage:
...
function displayhelp {
  echo "
  sysconfig.sh - display system information
  
  sysconfig.sh [OPTION]
OPTION:
  -h or --help  :: print this help and exist
  --host        :: print host name
  --domain      :: print domain name
  --ipconfig    :: print this machine's IP info and all network interfaces
  --os          :: print OS name and version
  --cpu         :: print hardware information
  --memory      :: print memory information
  --disk        :: print disk usage information
  --printer     :: print printer information
"
}

# This function will remove all the temp files created by the script
function cleanup {
  rm /tmp/sysinfo.$$ && rm /tmp/businfo.$$ && rm /tmp/memoryinfo.$$ && rm /tmp/cpuinfo.$$ && rm /tmp/sysreport.$$ && echo "Deleted Temporary Files"
  exit
}

...
# Command line arguments using while, $# and shift
customerInterfaceName=""
while [ $# -gt 0 ]; do
  case "$1" in
    -v )
      verbose="yes"
    ;;
    * )
      if [ "$customerInterfaceName" == "" ]; then
        customerInterfaceName="$1"
      fi
    ;;
  esac
...
if [ "$customerInterfaceName" == "" ]; then
    ...
    interface=$customerInterfaceName
...
interafaceArr=$(ifconfig | cut -d ' ' -f1| tr ':' '\n' | awk NF)

for interface in $interafaceArr; do
